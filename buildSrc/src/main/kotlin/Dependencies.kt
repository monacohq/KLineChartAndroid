...
object Dependencies {
    ...
    // third-party lib
    object DevLib {
        fun DependencyHandler.apiScarlet() {
            api(
                listOf(
                    ClassPaths.OKHTTP,
                    ClassPaths.KOTLIN_REFLECT,
                    ClassPaths.SCARLET,
                    ClassPaths.SCARLET_COROUTINE,
                    ClassPaths.SCARLET_LIFECYCLE,
                    ClassPaths.SCARLET_STREAM_ADAPTER,
                    ClassPaths.SCARLET_WEBSERVER
                )
            )
            api(ClassPaths.SCARLET_MOSHI) {
        ...
        fun DependencyHandler.calligraphy() {
            implementation(
                listOf(
                    ClassPaths.CALLIGRAPHY,
                    ClassPaths.VIEWPUMP
                )
            )
        }

        fun DependencyHandler.philology() {
            implementation(
                listOf(
                    ClassPaths.PHILOLOGY,
                    ClassPaths.VIEWPUMP
                )
            )
        }
        ...
        fun DependencyHandler.bugsnag() {
            implementation(ClassPaths.BUGSNAG_ANDROID)
        }

        fun DependencyHandler.koin() {
            implementation(
                listOf(
                    ClassPaths.KOIN_ANDROID,
                    ClassPaths.KOIN_ANDROIDX_SCOPE,
                    ClassPaths.KOIN_ANDROIDX_FRAGMENT,
                    ClassPaths.KOIN_ANDROIDX_VIEWMODEL
                )
            )
            androidTestImplementation(ClassPaths.KOIN_TEST)
            testImplementation(ClassPaths.KOIN_TEST)
        }

        fun DependencyHandler.logging() {
            implementation(ClassPaths.TIMBER)
        }

        fun DependencyHandler.retrofit() {
            implementation(ClassPaths.RETROFIT2_RETROFIT)
            implementation(ClassPaths.RETROFIT2_CONVERTER_MOSHI)
            testImplementation(ClassPaths.RETORFIT2_MOCKWEBSERVER)
        }

        fun DependencyHandler.mpAndroidChart() {
            implementation(ClassPaths.MP_ANDROID_CHART)
        }
    }
